...
public:
	...
	inline void loadOFF(const std::string& filename) {
		try {
			...
			throw std::exception((std::string("Error Loading OFF file: ") + std::string(e.what())).c_str());
		}
		recomputeNormals();
	}
	...
	void AABB_Mesh() {
		Vec3f min = Vec3f(9999.f, 9999.f, 9999.f);
		Vec3f max = -min;
		for (int i = 0; i < m_vertexPositions.size(); i++) {
			if (m_vertexPositions[i][0] < min[0]) min[0] = m_vertexPositions[i][0];
			if (m_vertexPositions[i][1] < min[1]) min[1] = m_vertexPositions[i][1];
			if (m_vertexPositions[i][2] < min[2]) min[2] = m_vertexPositions[i][2];
			if (m_vertexPositions[i][0] > max[0]) max[0] = m_vertexPositions[i][0];
			if (m_vertexPositions[i][1] > max[1]) max[1] = m_vertexPositions[i][1];
			if (m_vertexPositions[i][2] > max[2]) max[2] = m_vertexPositions[i][2];
		}
		m_AABB = AABB(min, max);
	}
